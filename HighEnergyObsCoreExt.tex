...
\subsection{Data Product Type Vocabulary}
...
We then propose to add the following product types to define the different response functions generally used in HE:

\begin{quote}
{\bf response-function}: A dataset that maps a physical quantity to an observable.
...
We finally propose a term to define a bundle, that includes the {\bf event-list} and associated ancillary data:
...
\begin{quote}
{\bf event-bundle}: An event-bundle dataset is a complex object containing an {\bf event-list} and multiple files or other substructures that are products necessary to analyze the {\bf event-list}.
\end{quote}
